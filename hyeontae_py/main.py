...
train = pd.read_csv('train_corr_removed(50, 0.7).csv', encoding='utf-8')
test = pd.read_csv('test_corr_removed(50, 0.7).csv', encoding='utf-8')

fe_train, fe_test = module.feature_engineering(train, test, 'OneHotEncoder', 'StandardScaler')
# print(fe_train.head())
# module.save_csv(fe_train, fe_test, 'final_train.csv', 'final_test.csv')
fe_train = fe_train.rename(columns = lambda x:re.sub('[^A-Za-z0-9_]+', '', x))
fe_test = fe_test.rename(columns = lambda x:re.sub('[^A-Za-z0-9_]+', '', x))

module.model(fe_train,fe_test, id, label, 10)
